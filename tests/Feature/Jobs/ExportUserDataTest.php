...
it('writes a ZIP containing data.json and all user media to the export disk', function () {
    ...
    Storage::put("users/{$user->id}/avatars/me.jpg", 'avatar-bytes');
    Storage::put("users/{$user->id}/posts/one.jpg", 'post-one-bytes');
    Storage::put("users/{$user->id}/originals/posts/one.mov", 'original-bytes');

    (new ExportUserData($user))->handle();

    $files = Storage::allFiles("gdpr-exports/{$user->id}");
    expect($files)->toHaveCount(1)
        ->and($files[0])->toEndWith('.zip');

    $localZip = tempnam(sys_get_temp_dir(), 'gdpr_test_').'.zip';
    file_put_contents($localZip, Storage::get($files[0]));
