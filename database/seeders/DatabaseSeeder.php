    ...
    public function run(): void
    {
        ...
        $posts->each(function (Post $post) use ($allUsers) {
            ...
            foreach ($likers as $user) {
                Like::factory()->for($post, 'likeable')->for($user)->create();
            }

            $post->comments->each(function (Comment $comment) use ($allUsers) {
                $commentLikers = $allUsers->random(rand(0, 3));
                foreach ($commentLikers as $user) {
                    Like::factory()->for($comment, 'likeable')->for($user)->create();
                }
            });
        });
